            ...
            <main className="lg:px-24 max-sm:px-4 flex flex-col gap-y-8 page-config--header">
                ...
                
                {
                    folders && folders.map((folder,idx)=>{
                        return(
                            <Link className="h-fit" key={folder.id}  href={`/account/folder/${folder.id}`}>
                ...
                
                {
                    creatingFolder && (
                        <form ref={formRef} className="border w-40 h-40 flex flex-col rounded-md p-2 justify-end" onSubmit={createFolder}>
                            ...
                            <input placeholder="folder name" autoFocus onBlur={()=>{setCreatingFolder(false)}} className="w-full bg-transparent outline-none border-b" name="name" type="text"/>
                            <button type="submit" className="w-full mt-2 active:bg-white active:bg-opacity-55">Create</button>
                        </form>
                    )
                }
